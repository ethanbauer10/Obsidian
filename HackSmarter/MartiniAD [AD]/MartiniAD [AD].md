...
However on this occasion this attack path is not valid because i already have a session as a user on the dc, so there is no point in remoting into a machine im already on!
...
```
And now using either the administrator or the athena.t0 account i can log in and get a session as a domain admin!
